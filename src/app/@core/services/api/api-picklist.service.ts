...
export class ApiPicklistService extends PicklistService{
    ...
    getAllPicklist(showErrorNotif:boolean, getTableDataParam: GetTableDataParam): Observable<HttpResponse<ApiResponse<Datatable<Array<Picklist>>>> | Observable<never>>{
        return this.apiService.post<ApiResponse<Datatable<Array<Picklist>>>>(environment.getAllPicklistUri, getTableDataParam).pipe(
            map((x: HttpResponse<ApiResponse<Datatable<Array<Picklist>>>>) => {
                //console.log("All picklist :",x);

                return this.handleResponse<ApiResponse<Datatable<Array<Picklist>>>>(showErrorNotif, x);
            }),
            catchError(error => {
    ...
    createPicklist(showErrorNotif:boolean, picklist:Picklist): Observable<HttpResponse<any> | Observable<never>>{
